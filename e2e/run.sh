...
ACT="${ACT:-act}"
WASM="${COMPONENT_WASM:-$COMPONENT_DIR/target/wasm32-wasip2/release/component_filesystem.wasm}"
...
TEST_DIR=$(mktemp -d /tmp/act-test-fs-XXXXXX)

"$ACT" serve "$WASM" --listen "[::1]:$PORT" &
HOST_PID=$!
trap "kill $HOST_PID 2>/dev/null; wait $HOST_PID 2>/dev/null; rm -rf $TEST_DIR" EXIT
